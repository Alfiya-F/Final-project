...
Для работы с данными была развернута СУБД PostgreSQL в Yandex Cloud (с помощью сервиса Managed Service for PostgreSQL). Датасет был представлен в виде CSV-файла и загружен в базу данных через DBeaver. Импорт данных прошел успешно, однако типы данных преимущественно были определены как строковые (varchar), поэтому было произведено преобразование типов данных на числа с плавающей точкой (float).

Проведены проверки качества исходных данных (см. запросы в папке "sql-queries", ["appendix-1"](./sql-queries/appendix-1.sql) в репозитории):
...
При построении витрины исключались записи с пропусками – с помощью фильтра WHERE rni IS NOT NULL (см. запрос в папке "sql-queries", ["appendix-2"](./sql-queries/appendix-2.sql) в репозитории).

Проведены проверки корректности витрины (см. запросы в папке "sql-queries", ["appendix-3"](./sql-queries/appendix-3.sql) в репозитории):
...
**📈 [Открыть интерактивный график в DataLens](https://datalens.yandex/ikwed84z1gja3)**

***

![График 2](./images/decline.png) 

**📈 [Открыть интерактивный график в DataLens](https://datalens.yandex/jlxff06fuxz44)**

***

Визуализация динамики рождаемости и смертности всех 200 городов сложна для восприятия, поэтому были отобраны 5 городов, которые чаще всего попадали в лидеры по приросту, и 5 городов, чаще всего попадавших в лидеры по убыли (см. запросы в папке "sql-queries", ["appendix-4"](./sql-queries/appendix-4.sql) в репозитории).
...
**📈 [Открыть интерактивный график в DataLens](https://datalens.yandex/vyvwo2y2k3oog)**
...
**📈 [Открыть интерактивный график в DataLens](https://datalens.yandex/5899tqtctahcq)**
...
**📈 [Открыть интерактивный график в DataLens](https://datalens.yandex/y1345wmw6xkmj)**
